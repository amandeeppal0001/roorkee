...
//         content: `<h6>${building.name}</h6>
//         <p>Carbon Score: ${building.carbonScore}</p>`,
//       });
...
function App() {
  ...
  // Load Mappls SDK scripts (map + plugins)
  useEffect(() => {
    ...
    loadScript(
      ...
      () => {
        loadScript(
          "https://apis.mappls.com/advancedmaps/api/7e9ae35043cd4d04737eb5cadc6e6903/map_sdk_plugins?v=3.0",
          "mappls-plugins",
          () => initializeMap()
        );
      }
  ...
  // Setup Mappls search (basic search box already in your code)
  ...
  // Setup "Default Place Search" like your HTML snippet
  const setupDefaultPlaceSearch = () => {
    const inputEl = document.getElementById("auto");
    if (!inputEl) return;
    ...
    new window.mappls.search(inputEl, optional_config, function callback(data) {
  ...
  return (
    <div className="app-container">
      {/* Map & Search */}
      <div style={{ position: "relative", flex: 1 }}>
        {/* First search box (from original code) */}
        <div
          id="searchBox"
          style={{  position: "absolute", top: "10px", left: "320px", zIndex: 1000 }}
        ></div>

        {/* New default place search input */}
        <input
          type="text"
          id="auto"
          name="auto"
